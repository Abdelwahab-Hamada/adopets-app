...
const Nav = () => {
  ...
  return (
    <div className="relative flex bg-white/40 w-fit mt-1 rounded">
        <button 
        className={
        location.pathname === '/adopets-app/'
        ? `pb-1 bg-white rounded`
        : ``
        }
        onClick={()=>navTo('/adopets-app/')}>         
            <HomeIcon className=" peer ... w-5"/>
            <span className="hidden absolute peer-hover:block whitespace-nowrap bg-black/50 text-white px-1 rounded right-full top-1 text-xs font-bold">Home Page</span>
        </button>
        <button 
        className={
        location.pathname === '/adopets-app/my-pets/'
        ? `pb-1 bg-white rounded`
        : ``
        }
        onClick={()=>{
